...
__kernel void burn(__global const uint32* data_info, __global const uint32* keys, __global uint32* hashes)
{
    ...
    for(i = 0; i< 1024; i++){
        ...
        hashes[gid * 8 + 0] = ctx.state[0];
        hashes[gid * 8 + 1] = ctx.state[1];
        hashes[gid * 8 + 2] = ctx.state[2];
        hashes[gid * 8 + 3] = ctx.state[3];
        hashes[gid * 8 + 4] = ctx.state[4];
        hashes[gid * 8 + 5] = ctx.state[5];
        hashes[gid * 8 + 6] = ctx.state[6];
        hashes[gid * 8 + 7] = ctx.state[7];
    }
}
